...
const validateAmounts = (items, amounts) => {
...
const RepairPaymentModal = ({ open, onClose, loan, initialItems, onPaid }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await api.post(`/api/loantool/repair/${loan.id}/pay`, { adminUser, cost: Number(totalCost) });
      try { alert?.show?.({ severity: 'success', message: 'Reparaciones pagadas correctamente.', autoHideMs: 4000 }); } catch (error_) { console.debug(error_); }
      onPaid?.();
      onClose?.();
    } catch (err) {
      ...
      try {
        ...
        alert?.show?.({ severity: 'error', message: msg, autoHideMs: 8000 });
      } catch (error_) { console.debug(error_); }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 60 }}>
      ...
      <div
        ...
        style={{ background: '#fff', borderRadius: 8, width: '90%', maxWidth: '70%', padding: 20, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ marginTop: 12 }}>
          ...
          {!loading && items.length === 0 && <p>No hay items para reparar.</p>}
          {!loading && items.length > 0 && (
              <div style={{ display: 'grid', gap: 12 }}>
                {items.map(it => {
                  const displayName = it?.idTool?.toolName || it?.toolName || it?.name || it?.description || `Item ${it.id}`;
                  ...
                  return (
                    <div key={it.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 10, border: '1px solid #e6e6e6', borderRadius: 6 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, minWidth: 210 }}>
                        <label htmlFor={`repair-amount-${it.id}`} style={{padding: 8, justifyContent: 'center', fontSize: 16, color: '#000000ff' }}>Monto reparación</label>
                        <input
                          id={`repair-amount-${it.id}`}
